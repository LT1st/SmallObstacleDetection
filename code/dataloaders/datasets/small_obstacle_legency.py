...
class SmallObs(data.Dataset):
	...
	def __init__(self, args, image_paths, split='train'):
		'''
		:param args:
		:param image_paths: 每张的图像路径 list
		:param split: 分割标志
		'''
...
class SmallObs_RGBD(data.Dataset):
	...
	def __getitem__(self, index):
      ...
      dtype=int64)
		...
		# 拼接
		_img = np.concatenate((np.asarray(_img), np.asarray(_depth)), axis=2)
...
if __name__ == '__main__':
	...
	cityscapes_train = SmallObs(args=args, image_paths=path, split='train')
	'''
	报错 已修改
	由于文件路径不符合格式，迭代器错误
	'''
